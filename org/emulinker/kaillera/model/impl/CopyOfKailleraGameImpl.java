...
public final class CopyOfKailleraGameImpl implements KailleraGame
{
	private static Log	log							= LogFactory.getLog(CopyOfKailleraGameImpl.class);
	...
	public CopyOfKailleraGameImpl(int gameID, String romName, KailleraUserImpl owner, KailleraServerImpl server, int bufferSize, int timeoutMillis, int desynchTimeouts)
	{
		this.id = gameID;
	...
	public synchronized int join(KailleraUser user) throws JoinGameException
	{
		...
		//{
			...
			announce("Help: " + getServer().getReleaseInfo().getProductName() + " v" + getServer().getReleaseInfo().getVersionString() + ": " + getServer().getReleaseInfo().getReleaseDate() + " - Visit: http://smashboards.com/threads/emulinker-x-v2-0.390887/", user);
	        announce("*****************************************", user);
	        announce("Update: This server is using Emulinker X", user);
	        announce("By: Firo and Agent 21 iXi", user);
	        announce("*****************************************", user);
			/*
