			...
			if(doCommand){
				...
				if(((Chat) message).getMessage().equals("/alivecheck")){
		            ...
		            try {
		             ...
		             try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", "********Server Rules********")); } catch(Exception e) {}
					 try { Thread.sleep(20); } catch(Exception e) {}
					 try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", stringBuffer.toString())); } catch(Exception e) {}
					 try { Thread.sleep(20); } catch(Exception e) {}
		            }
