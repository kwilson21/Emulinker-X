...
public class AdminCommandAction implements V086Action
{
	...
	public boolean isValidCommand(String chat){
		...
		else if (chat.startsWith(COMMAND_UNSILENCE))
		{
			return true;
		}
		return false;
	...
	private void processHelp(String message, KailleraServerImpl server, KailleraUserImpl admin, V086Controller.V086ClientHandler clientHandler) throws ActionException, MessageFormatException
	{
		...
		try { Thread.sleep(20); } catch(Exception e) {}
		clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", "/unsilence to unsilence a silenced user")); //$NON-NLS-1$ //$NON-NLS-2$
		try { Thread.sleep(20); } catch(Exception e) {}
    ...
    private void processUnscramble(String message, KailleraServerImpl server, KailleraUserImpl admin, org.emulinker.kaillera.controller.v086.V086Controller.V086ClientHandler clientHandler)
        ...
        {	
            ...
            if(message.startsWith("/unscrambletime"))
            {
    			...
    			throw new ActionException("Unscramble Error: You must use the following commands: /unscramblereset, /unscrambleon, /unscramblepause, /unscramblesave & /unscrambletime");
        }
